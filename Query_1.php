...
<?php
	...
	echo "<h2>How many Netflix shows were released in 2020 that were streamed by Netflix in the given country?</h2><br>";
	...
	if (empty($released_country)) {
		echo "empty <br><br>";
	
	} else {
	
		echo $released_country."<br><br>";
	
		//Prepare a statement that we can later execute. The ?'s are placeholders for
		//parameters whose values we will set before we run the query.
		if ($stmt = $conn->prepare("CALL Query_1(?)")) {
		...
		$stmt->bind_param("s", $released_country);
		...
		if ($stmt->execute()) {
			...
			if (($result) && ($result->num_rows != 0)) {
				...
				echo "<tr><th> The Number of Netflix Shows </th></tr>";
